...
f.doubleclickdeleteordestroy::
  Similar to f.deleteordestroy, but only triggers if double-clicked within the time given by the `ConstrainedMoveTime` variable.
...
f.menuorclose `string`::
  If clicked once, behaves like `f.menu` above.  If double-clicked, behaves like `f.deleteordestroy`.  Double-click time is specified by the `ConstrainedMoveTime` variable.
...
f.raisemoveorsqueeze::
  Raise the window on click.  If the window is dragged, the window moves like `f.movepack`.  If double-clicked within the `ConstrainedMoveTime` period, the window is squeezed.
